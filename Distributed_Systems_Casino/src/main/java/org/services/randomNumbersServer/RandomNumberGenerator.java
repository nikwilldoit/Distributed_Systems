package org.services.randomNumbersServer;
...
public class RandomNumberGenerator extends Thread {
    ...
    public RandomNumberGenerator(int bufferSize,int port) {
        if  (bufferSize <= 100) {
    ...
    static class ResponseThread extends Thread {
        ...
        public void run()  {
            ...
            synchronized (randomNumbersQueue) {
                while (randomNumbersQueue.isEmpty()) {
                    try {
                        randomNumbersQueue.wait();
